...
**Obsidian 零配置，装完即用** — 无需安装 Obsidian 插件，无需 Local REST API，无需服务器。Notion 仅需一次约 1 分钟的设置。
...
勾选 Notion 输出后，需要一次性完成以下设置（扩展内引导页有同样的指引），字段会在第一次 Clip 时自动创建：

1. 打开 Notion 的 **Settings → Connections** 页面，点击右上角 **New connection**

   <img src="assets/notion-step1.png" width="600" alt="Notion Connections 页面，点击 New connection">

2. 填写名称（如 `Bilibili clipper`），Authentication method 选 **Access token**，选择你的 workspace，点击 **Create connection**

   <img src="assets/notion-step2.png" width="400" alt="新建 connection 弹窗，选择 Access token">

3. 在创建好的 connection 的 **Configuration** 页，复制 **Access token**，粘贴到扩展设置中

   <img src="assets/notion-step3.png" width="600" alt="复制 Access token">

4. 打开（或新建）要写入的 database 页面，点击右上角 **⋯** → **Connections**，搜索并选中刚创建的 connection

   <img src="assets/notion-step4.png" width="500" alt="在 database 页面把 connection 连接进来">

5. 回到扩展设置，点击 **Connect**，在下拉框中选择这个 database——完成
...
[查看隐私政策](https://echore.github.io/bilibili-to-obsidian/privacy-policy.html) — 扩展不收集任何用户数据；仅在你勾选 Notion 输出时，笔记内容（标题、字幕、视频链接）才会发送至 Notion 官方 API，其余数据全部仅存本地。
