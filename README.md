...
- **Notion 设置**（勾选 Notion 后显示）— 支持写入 Notion：创建 connection → 连接 database → 扩展内一键选择，字段自动创建（步骤见下方指南）

### Notion 设置指南

勾选 Notion 输出后，需要一次性完成以下设置（扩展内引导页有同样的指引）：
...
打开任意有 CC 字幕的 B 站视频，页面**右上角**会出现紫色 **Clip 浮动栏**，点击 **Clip** 即可。笔记会保存到 `<目标文件夹>/<视频标题>.md`，并自动在 Obsidian 中打开。
