...
勾选 Notion 输出后，需要一次性设置（约 1 分钟）：创建一个 Notion connection → 把目标 database 连接给它 → 回扩展点 **Connect** 选择即可。

**带截图的图文步骤在安装后自动弹出的引导页里**，跟着点一遍就行。没有现成的 database？两种方式任选：[一键复制本模板](https://fifree.notion.site/37b942e6a59280eba812e54321b78851?v=37b942e6a59280c78fee000c7513971f)（字段已配好，复制即用），或新建一个空库（所需字段会在第一次 Clip 时自动创建）。
